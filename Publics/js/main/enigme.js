window.onload = function() {
    // function myMouse () {  
    //     const elem = document.getElementById("curseur"); 
    //     var a = $(window).width()
    ...
    let deadline = new Date('2023/03/21').getTime();
    ...
    $('#enigme-form').submit(function(e) {
        ...
        if (current_enigme < data_enigme.length) {
            ...
            if (quiz_enigme.reponse == $('#reponse').val()) {
                $('#titre_popup').text('Wow!! Bravos vous aver la bonne reponse! on va voir si vous pouvez resoudre la prochinde question!').css('color', 'bleu');
                $('#btn-popup').text('qustion suivante >');
                $('#box-popup').show();
                current_enigme++;
                badResponse = false;
            } else {
                $('#titre_popup').text('C\'est pas grave n\'ete pas le seul!recomencez encore! reflechissez Bien!').css('color', 'red');
                $('#btn-popup').text('réessayez');
                $('#box-popup').show();
                badResponse = true
            }
            if (current_enigme == data_enigme.length) {
                $('#titre_popup').text('Félicitation vous avez reléver le defis vous ete vraiment fort!').css('color', 'orange');
